 ...
 * Original files:
 * - /gh/hifocus/merger@0.20/assets/js/jQuery/jquery.min.js
 * - /gh/hifocus/merger@0.20/assets/js/jquery-qrcode/jquery-qrcode.min.js
 * - /gh/hifocus/merger@0.20/assets/js/function.min.js
 * - /gh/hifocus/merger@0.20/assets/js/font_974919_xgtacm93vxp.min.js
 *
...
if (!1 !== multilingual)
    if (/zh-CN|zh-cn|zh-Hans|zh-hans|cn/i.test(userLang)) {
        if ("string" == typeof myname_hans)
            var myname = myname_hans
    } else if (/zh-TW|zh-HK|zh-tw|zh-hk|zh-Hant|zh-hant|tw|hk/i.test(userLang)) {
        if ("string" == typeof myname_hant)
            var myname = myname_hant
    } else if ("string" == typeof myname_eng)
        var myname = myname_eng;
var firstchar = myname.charAt(0)
  , lastchar = myname.charAt(myname.length - 1)
  , english = /^[A-Za-z0-9]*$/;
if (english.test(firstchar) || english.test(lastchar))
    var finalname = " " + myname + " "
      , finalsub = subtitle.replace(myname, finalname)
      , spacing = !0;
else
    var finalname = myname;
var finalname_eng = " " + myname;
if (null == usage || "" === usage || "payment" !== usage && "donate" !== usage) {
    var usage = "payment";
    console.log("%c The usage variable is not defined correctly", "color: red")
}
if ("undefined" == typeof branding || null === branding || "" === branding || !0 !== branding && "true" !== branding)
    var aftertitle = "";
else if (!0 === branding || "true" === branding)
    var aftertitle = " | Merger";
if (!1 === multilingual)
    var finaltitle = title
      , wechatscan = "微信扫一扫 向" + finalname + "支付"
      , tenpayscan = "手机QQ扫一扫 向" + finalname + "支付"
      , alipayscan = "支付宝扫一扫 向" + finalname + "支付"
      , payto = ""
      , presshold = "长按识别二维码 向" + finalname + "支付"
      , notavail = "🚫 目前没有可用的支付方式"
      , myname_hant = myname
      , myname_hans = myname
      , myname_eng = myname;
else if (/zh-CN|zh-cn|zh-Hans|zh-hans|cn/i.test(userLang)) {
    if (document.write("<style>body { font-family: 'Segoe UI', -apple-system, system-ui, BlinkMacSystemFont, Roboto, 'PingFang SC', 'Hiragino Sans GB', 'Microsoft YaHei', 'Helvetica Neue', sans-serif; }</style>"),
    "payment" === usage)
        var method = "付款";
    if ("donate" === usage)
        var method = "捐赠";
    document.title = "向" + finalname + method + aftertitle;
    var trans_wx = "微信", trans_ali, trans_tp, finaltitle = "选择你的" + method + "方式", finalsub = "跟随下一步的指示以向" + finalname + method, trans_wx, scan, presshold = "长按识别二维码", payto = " 向" + finalname + method, notavail = "🚫 目前没有可用的" + method + "方式", wechatscan = (trans_wx = "微信") + (scan = "扫一扫"), tenpayscan = (trans_tp = "QQ手机版") + scan, alipayscan = (trans_ali = "支付宝") + scan
} else if (/zh-TW|zh-HK|zh-tw|zh-hk|zh-Hant|zh-hant|tw|hk/i.test(userLang)) {
    if (document.write("<style>body { font-family: 'Segoe UI', -apple-system, system-ui, BlinkMacSystemFont, Roboto, 'PingFang TC', 'Hiragino Sans CNS', 'Microsoft JhengHei', 'Helvetica Neue', sans-serif; }</style>"),
    "payment" === usage)
        var method = "付款";
    if ("donate" === usage)
        var method = "捐贈";
    document.title = "向" + finalname + method + aftertitle;
    var trans_wx, trans_ali, trans_tp, finaltitle = "選擇你的" + method + "方式", finalsub = "跟隨下一步的指示以向" + finalname + method, scan, notavail = "🚫 目前沒有可用的" + method + "方式", wechatscan = (trans_wx = "WeChat ") + (scan = "掃一掃"), tenpayscan = (trans_tp = "QQ手機版") + scan, alipayscan = (trans_ali = "支付寶") + scan, presshold = "長按識別二維碼", payto = " 向" + finalname + method
} else {
    if ("payment" === usage)
        var method = "Pay"
          , method_t = "Payment";
    ...
    document.write("<style>body { font-family: 'Segoe UI', -apple-system, system-ui, BlinkMacSystemFont,  Roboto, 'Helvetica Neue', sans-serif; }</style>");
    var trans_wx = "WeChat"
      , trans_ali = "AliPay"
      , trans_tp = "QQ Mobile"
      , trans_pm = "pay"
      , trans_dn = "donate"
      , finaltitle = "Choose Your " + method_t + " Method"
      , finalsub = "Then follow the instruction<br>to proceed a " + method_t.replace(method_t.charAt(0), method_t.charAt(0).toLowerCase()) + " to" + finalname_eng
      , scanhint = "Scan the QR Code to " + method + finalname_eng
      , presshold = method.replace(method.charAt(5), "") + method.charAt(5).replace("e", "") + "ing to" + finalname_eng + ":<br><span style='font-weight:400'>Press and hold to recognise the Qrcode</span>"
      , scan = "Scan the QR Code on "
      , payto = ""
      , notavail = "🚫 Currently no " + method_t.replace(method_t.charAt(0), method_t.charAt(0).toLowerCase()) + " method available"
      , wechatscan = scan + trans_wx
      , tenpayscan = scan + trans_tp
      , alipayscan = scan + trans_ali
}
var error_num = 0;
if ("undefined" == typeof tenpay || null === tenpay || "" === tenpay) {
    document.getElementById("depends").removeChild(document.getElementById("tenpaybtn")),
    error_num += 1;
    var notenpay = !0
}
if ("undefined" == typeof wechat || null === wechat || "" === wechat) {
    document.getElementById("depends").removeChild(document.getElementById("toclick")),
    error_num += 1;
    var nowechat = !0
}
if ("undefined" == typeof alipay || null === alipay || "" === alipay) {
    document.getElementById("depends").removeChild(document.getElementById("alipaybtn")),
    error_num += 1;
    var noalipay = !0
}
if ("undefined" == typeof paypal || null === paypal || "" === paypal)
...
if (navigator.userAgent.match(/Alipay/i))
    if (!0 === noalipay)
        ...
        "showqrcode" != location.href.substr(location.href.lastIndexOf("#") + 1) || selected || (document.getElementById("showqrcode").style.display = "none");
    else {
    ...
    document.getElementById("qrcodeclose").onclick = function() {
        ...
        "flex" == document.getElementById("showqrcode").style.display && (document.getElementById("showqrcode").style.display = "")
    }
    ;
if (4 === error_num) {
...
function removal() {
    document.getElementById("h").removeChild(document.getElementById("i")),
    document.getElementById("pending").removeChild(document.getElementById("depends")),
...
function showqrcode() {
    "showqrcode" == location.href.substr(location.href.lastIndexOf("#") + 1) && "yes" == selected && (document.getElementById("showqrcode").style.display = "flex"),
    document.getElementById("currentqrcode").innerHTML.includes("img") && (document.getElementById("currentqrcode").getElementsByTagName("img")[0].setAttribute("id", "todel"),
